...
export function setupConfig(env){
 ...
 const divisions=[1,2].map(id=>({id,name:env[`DIV_${id}_NAME`]||`${id}ª divisão`,guild:env[`DIV_${id}_GUILD_ID`],memberRole:env[`DIV_${id}_MEMBER_ROLE_ID`]||undefined,adminRole:env[`DIV_${id}_ADMIN_ROLE_ID`]||undefined}));
...
export async function runSetup(env=process.env){const log=reporter();let client,server;const deadline=setTimeout(()=>{log.line('BLOQUEADO','Tempo máximo do instalador excedido.');process.exit(1);},120000);
 try{
  ...
  await log.test('Instalador: registrar /criar nos dois servidores',()=>registerCreate(client,config.divisions));
