...
export async function registerCreate(client,divisions){for(const d of divisions){const g=await client.guilds.fetch(d.guild);await g.commands.create(createCommand(divisions));}}
...
export async function provisionGuild(g,d){
 ...
 async function role(key,emoji,suffix){const configured=key==='member'?d.memberRole:key==='admin'?d.adminRole:undefined;const name=`${tag} • ${emoji} ┃ ${suffix}`;
 ...
 const values={GUILD_ID:g.id,MEMBER_ROLE_ID:member.id,ADMIN_ROLE_ID:admin.id,VERIFICATION_CHANNEL_ID:channelMap.get('verification').id,WANTED_CHANNEL_ID:channelMap.get('wanted').id,TICKETS_CHANNEL_ID:channelMap.get('tickets').id,REPORTS_CHANNEL_ID:channelMap.get('reports').id,TICKET_CATEGORY_ID:categoryMap.get('tickets-category').id,STAFF_CHANNEL_ID:channelMap.get('staff-text').id};
 return {env:Object.entries(values).map(([key,value])=>`DIV_${d.id}_${key}=${value}`).join('\n')+'\n',inventory:`ROKUHARA · Divisão ${d.id} · servidor ${g.id}\n8 categorias • 24 textos • 6 calls • 10 cargos\n\n${inventory.join('\n')}\n\n${notes.join('\n')}\n\nOs cargos de patente são visuais e de atribuição manual. Não alteram a patente no banco nem liberam canais por si mesmos. Membro verificado libera acesso; Equipe abre áreas da administração e tickets. Os canais de avisos, regras, agenda e registros são espaços para a equipe preencher; não há publicação automática nesses canais extras.\n`};

}
export function createProvisionHandler(client,divisions){const locks=new Set();return async i=>{
 ...
 try{
  ...
  await i.editReply({content:'Estrutura pronta e decorada! Importe apenas o arquivo .env no Render → Environment. Execute /criar também no outro servidor. Depois use npm start e publique os botões em Bot e histórico. O cargo Equipe deve ser atribuído manualmente aos administradores que atendem tickets.',files:[new AttachmentBuilder(Buffer.from(result.env),{name:`divisao-${id}.env`}),new AttachmentBuilder(Buffer.from(result.inventory),{name:`ids-completos-divisao-${id}.txt`})],allowedMentions:{parse:[]}});
