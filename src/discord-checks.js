...
export async function validateDiscord(client,config,log){
 ...
 for(const d of config.divisions){
  ...
  for(const [suffix,id]of [['VERIFICATION_CHANNEL_ID',d.verification],['WANTED_CHANNEL_ID',d.wanted],['TICKETS_CHANNEL_ID',d.tickets],['REPORTS_CHANNEL_ID',d.reports],...(d.staffChannel?[['STAFF_CHANNEL_ID',d.staffChannel]]:[])]){
   await log.test(`${prefix}_${suffix}: canal e permissões`,async()=>{
    ...
    if(suffix==='VERIFICATION_CHANNEL_ID')requireCondition(ch.permissionsFor(g.roles.everyone)?.has(P.ViewChannel),`${prefix}: o canal de verificação precisa ser visível a @everyone.`);
