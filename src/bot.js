...
export function createBot(pool,config){
 ...
 async function execute(job,c){
  ...
  if(job.kind==='sync-role'){
   ...
   const member=(await c.query('SELECT * FROM members WHERE discord_id=$1',[p.discordId])).rows[0];
   for(const d of config.divisions){
    const g=await guild(d.guild);const user=await g.members.fetch(p.discordId).catch(e=>{if(e.code===10007)return null;throw e;});if(!user)continue;
    const wanted=member?.verified && (bothDivisions(member)||member.division===d.id);
    if(wanted)await user.roles.add(d.memberRole,'Verificação IFJ');else await user.roles.remove(d.memberRole,'IFJ cancelado ou outra divisão');
   }return;
  }
 ...
 async function work(){
  ...
  try{
   for(let n=0;n<10;n++){
    ...
    }catch(e){if(!picked)throw e;const attempts=picked.attempts+1;await pool.query('UPDATE jobs SET attempts=$1,status=$2,error=$3,next_at=$4 WHERE id=$5',[attempts,attempts>=8?'failed':'pending',`Discord/operação: ${e.code||e.name}. Verifique canais, cargos, permissões e conexão.`,new Date(Date.now()+Math.min(300000,2000*2**attempts)),picked.id]);}
   }
 ...
 async function start(){
  ...
  await new Promise((resolve,reject)=>{
   const finish=err=>{clearTimeout(timeout);client.off('clientReady',onReady);if(err)reject(err);else resolve();};
   ...
   client.once('clientReady',onReady);client.login(config.token).catch(finish);
  });
 }
 client.on('error',e=>console.error('Conexão Discord:',e.code||e.name));
 return {ready,setup,closeTicket,client,handleInteraction:interact,start,activate,validate:async log=>{await validateDiscord(client,config,log);await log.test('Discord: registrar comando /criar',()=>registerCreate(client,config.divisions));},stop:async()=>{activated=false;clearInterval(timer);await client.destroy();},work};
}
