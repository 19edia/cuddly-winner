...
before(async()=>{
 ...
 bot=createBot(pool,config);app=createApp(pool,config,bot);
...
test('worker applies roles only in matching division and removes both after revocation',async()=>{
 ...
 bot.client.guilds.fetch=async guildId=>({members:{fetch:async userId=>({roles:{add:async role=>added.push([guildId,userId,role]),remove:async role=>removed.push([guildId,userId,role])}})}});
...
test('allies verify from either guild, synchronize both roles, and revocation removes both',async()=>{
 ...
 for(const d of config.divisions){
  ...
  try{
   ...
   fresh.client.isReady=()=>true;fresh.client.guilds.fetch=async guildId=>({members:{fetch:async userId=>({roles:{add:async()=>added.push([guildId,userId]),remove:async()=>removed.push([guildId,userId])}})}});
   await fresh.work();
  }finally{await fresh.stop();}
 }
 for(const d of config.divisions)assert.ok(added.some(([g,u])=>g===d.guild&&u===m.discord_id));
 const fresh=createBot(pool,config);fresh.client.isReady=()=>true;fresh.client.guilds.fetch=async guildId=>({members:{fetch:async userId=>({roles:{add:async()=>added.push([guildId,userId]),remove:async()=>removed.push([guildId,userId])}})}});
 try{
