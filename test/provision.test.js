...
test('provision creates private internal text/voice, public entry, and reuses its resources',async()=>{
 ...
 for(const ch of f.channels.values()){
  ...
  const publicEntry=ch.name.includes('PORTAL')||ch.name.includes('INFORMAÇÕES')||['verification','tickets','welcome','rules','guide','news','hierarchy','alliances'].some(key=>ch.topic===`IFJ-D1:${key}`);
...
test('installer validates distinct guilds and registration preserves unrelated commands',async()=>{
 ...
 const registered=[];await registerCreate({guilds:{fetch:async id=>({commands:{create:async data=>registered.push({id,data})}})}},divisions);assert.equal(registered.length,2);assert.equal(registered[0].data.name,'criar');assert.equal(registered[0].data.default_member_permissions,String(P.Administrator));
});
